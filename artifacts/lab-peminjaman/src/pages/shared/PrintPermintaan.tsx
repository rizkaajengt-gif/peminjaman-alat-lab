...
function TTDBox({ label, nama, nimNip, tandaTangan }: { label: string; nama?: string; nimNip?: string | null; tandaTangan?: string | null }) {
  return (
...
export default function PrintPermintaan() {
  ...
  const plp = (data as any).plp as any;
  const nimNip = user?.nim || user?.nip || null;
  const plpNip = plp?.nip || null;
  const ditujukanKe = data.tujuan === "plp" ? `PLP: ${plp?.nama || "-"}` : "Gudang";
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[794px] mx-auto p-6 print:p-6 text-[13px]">
        ...
        {/* TTD 2 kolom: Pemohon + PLP/Gudang */}
        <div className="grid grid-cols-2 gap-8 mt-6">
          ...
          />
          <TTDBox
            label={data.tujuan === "plp" ? "PLP Laboratorium" : "Petugas Gudang"}
            nama={plp?.nama || ""}
            nimNip={plpNip}
            tandaTangan={plp?.tandaTangan || null}
          />
        </div>
