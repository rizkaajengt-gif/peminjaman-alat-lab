...
export default function PrintPeminjamanPhantom() {
  const [, params] = useRoute("/print/peminjaman-phantom/:id");
  const itemId = params?.id ? parseInt(params.id) : 0;
  const { data, isLoading } = useQuery({
    queryKey: ["/api/peminjaman-phantom", itemId],
    queryFn: () => customFetch(`/api/peminjaman-phantom/${itemId}`),
    enabled: !!itemId,
  });
  ...
  useEffect(() => {
    if (d) document.title = `Peminjaman Phantom ${d.noPeminjaman}`;
  }, [d]);
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="print:hidden bg-slate-100 border-b px-6 py-3 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Preview cetak — Surat Peminjaman Phantom</p>
      ...
      <div className="max-w-[794px] mx-auto p-6 print:p-6 text-[13px]">
        ...
        <div className="text-center mb-4">
          <h2 className="text-[14px] font-bold uppercase tracking-widest">SURAT PEMINJAMAN PHANTOM LABORATORIUM</h2>
        </div>
          ...
          <table className="w-full border border-slate-200 rounded-lg overflow-hidden">
            <tbody>
              {[
                ...
                ["Tgl Pinjam", `${fmt(d.tanggalPinjam)}${d.jamPinjam ? ` pukul ${d.jamPinjam}` : ""}`],
                ["Tgl Kembali", `${fmt(d.tanggalKembali)}${d.jamKembali ? ` pukul ${d.jamKembali}` : ""}`],
        ...
        {/* Tabel Phantom */}
        <h3 className="font-bold text-[11px] mb-1.5 uppercase tracking-wide text-slate-700">Daftar Phantom yang Dipinjam</h3>
        <table className="w-full border border-slate-200 mb-4 text-[11px]">
          <thead>
            <tr className="bg-slate-100 text-left">
              ...
              <th className="px-3 py-2 font-semibold border-b border-r border-slate-200">Nama Phantom</th>
          ...
          <tbody>
            {d.items?.map((item: any, i: number) => (
              <tr key={i} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-3 py-1.5 font-mono text-[10px]">{item.phantom?.kode}</td>
                <td className="px-3 py-1.5 font-medium">{item.phantom?.nama}</td>
                <td className="px-3 py-1.5 text-center font-bold">{item.jumlah}</td>
                <td className="px-3 py-1.5">{item.phantom?.satuan}</td>
        ...
        {d.kondisiKembali && (
          <div className="mb-4 p-2.5 bg-blue-50 border border-blue-200 rounded-lg text-[11px]">
            ...
            <span className="text-blue-700 capitalize">{d.kondisiKembali}</span>
          </div>
        )}
